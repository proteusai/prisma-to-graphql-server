...
export function parseString(
  ...
  if (Array.isArray(value)) {
    throw new Error(
      `Invalid "${optionPropertyName}" option value "${value}" provided for ${GENERATOR_NAME} generator.`,
    );
  }
  return value;
}
...
export function parseStringArray<TAllowedValue extends string>(
  ...
  if (typeof stringArray === "string") {
    if (!stringArray.includes(",")) {
      throw new Error(
        `Invalid "${optionPropertyName}" value "${stringArray}" provided for ${GENERATOR_NAME} generator.`,
      );
    }
  ...
  if (allowedValues) {
    for (const option of parsedArray) {
      if (!allowedValues.includes(option as any)) {
        throw new Error(
          `Invalid "${optionPropertyName}" option value "${option}" provided for ${GENERATOR_NAME} generator.`,
        );
      }
    }
  }
...
export function parseStringEnum<TAllowedValue extends string>(
  ...
  if (!allowedValues.includes(stringEnum as any)) {
    throw new Error(
      `Invalid "${optionPropertyName}" option value "${stringEnum}" provided for ${GENERATOR_NAME} generator.`,
    );
  }
